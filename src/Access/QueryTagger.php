...
class QueryTagger implements ContainerInjectionInterface {
  ...
  public function tagQuery(SelectInterface $query) : void {
    ...
    if (!$null_query_or) {
      ...
      $all_or = $query->orConditionGroup();
      $all_or->notExists($null_query);
      $all_or->exists($existence);
      $query->condition($all_or);
    }
    else {
      $null_query_or = $query->getMetaData('islandora_hierarchical_access_tagged_null_query_or');
    }
  ...
  protected function getMediaQuery($tagged = FALSE): SelectInterface {
    $query = $this->database->select('media', 'm')
      ...
      ->addMetaData('islandora_hierarchical_access_type', 'media');
  ...
  protected function getNodeQuery($tagged = FALSE): SelectInterface {
    $query = $this->database->select('node', 'n')
      ...
      ->addMetaData('islandora_hierarchical_access_type', 'node');
