...
namespace Drupal\access_control_model\Access;
...
class QueryTagger {
  ...
  protected function getMediaQuery($tagged = FALSE) : SelectInterface {
    ...
    $lut_alias = $query->join(LUTGeneratorInterface::TABLE_NAME, 'lut', '%alias.mid = m.mid');
  ...
  public function tagFile(AlterableInterface $query) {
    ...
    $file_tables = $this->entityTypeManager->getStorage('file')->getTableMapping()->getTableNames();
    ...
    foreach ($query->getTables() as $info) {
      ...
      elseif (in_array($info['table'], $file_tables)) {
        ...
        $existential_query->condition("$alias.fid", $this->getBaseMediaQuery(), 'NOT IN');
  ...
  protected function getNodeQuery($tagged = FALSE) : SelectInterface {
    ...
    $lut_alias = $query->join(LUTGeneratorInterface::TABLE_NAME, 'lut', '%alias.nid = n.nid');
  ...
  public function tagMedia(AlterableInterface $query) {
    ...
    $media_tables = $this->entityTypeManager->getStorage('media')->getTableMapping()->getTableNames();
